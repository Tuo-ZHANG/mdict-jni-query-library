...
Java_com_example_mdictjnilibrary_MainActivity_entryPoint(JNIEnv *env, jobject thiz, jstring argument1, jstring argument2) {
    // TODO
    const int argc = 3;
    const char *placeholder = "placeholder";
    char *argv[argc];
    const char *path = env->GetStringUTFChars(argument1, nullptr);
    const char *word = env->GetStringUTFChars(argument2, nullptr);
...
//    }
    ...
    env->ReleaseStringUTFChars(argument1, path);
    env->ReleaseStringUTFChars(argument2, word);
